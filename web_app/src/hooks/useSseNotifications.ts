...
export function useSseNotifications(options: UseSseNotificationsOptions) {
  ...
  useEffect(() => {
    ...
    // 어떤 형태로 오든 수신 원본을 먼저 기록
    const logRawSseEvent = (e: MessageEvent, source: 'typed' | 'onmessage') => {
      debugLog('[SSE][RAW]', {
        source,
        type: e.type,
    ...
    const handleMessage = (e: MessageEvent) => {
      ...
      try {
        ...
        logger.error('메시지 파싱 실패: ' + (e as any)?.data, error);
      }
    };

    // 기본 message 이벤트도 수신 로그 남김
    // (서버가 event: 없이 보내는 경우를 대비)
    eventSource.onmessage = (e) => {
      logRawSseEvent(e, 'onmessage');
      try {
        const envelope = JSON.parse(e.data) as NotificationEnvelope;
        logger.dev('SSE onmessage 전체:', envelope);
      } catch (error) {
        logger.error('onmessage 파싱 실패: ' + (e as any)?.data, error);
      }
    };
    ...
    debugLog('[useSseNotifications] SSE 연결 시작, 등록된 이벤트:', SSE_EVENT_NAMES);
