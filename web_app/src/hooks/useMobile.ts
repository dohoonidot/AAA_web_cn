...
/**
 * 키보드 높이 및 상태 감지 훅 (모바일용)
 * visualViewport API 사용 + 폴백 메커니즘
 * 하단 브라우저 UI (주소창/툴바)도 감지
 */
export const useKeyboardHeight = () => {
  const [keyboardHeight, setKeyboardHeight] = useState(0);
  const [isKeyboardOpen, setIsKeyboardOpen] = useState(false);
  const [browserBottomBar, setBrowserBottomBar] = useState(0);

  useEffect(() => {
    if (!('ontouchstart' in window)) return;

    // 키보드 임계값: 이 값 이상이면 키보드가 열린 것으로 판단
    ...
    const updateKeyboardHeight = () => {
      ...
      if (vv) {
        // visualViewport API 사용 (권장)
        const offsetTop = vv.offsetTop || 0;
        bottomOffset = Math.max(0, window.innerHeight - vv.height - offsetTop);
      } else {
        // 폴백: window.innerHeight 변화 감지
        const screenHeight = window.screen.availHeight;
        bottomOffset = Math.max(0, screenHeight - window.innerHeight);
      }

      // 키보드 vs 하단 브라우저 UI 구분
      const isKbOpen = bottomOffset >= KEYBOARD_THRESHOLD;
      const kbHeight = isKbOpen ? bottomOffset : 0;
      const browserBar = (!isKbOpen && bottomOffset > 0) ? bottomOffset : 0;

      setKeyboardHeight(kbHeight);
      setIsKeyboardOpen(isKbOpen);
      setBrowserBottomBar(browserBar);
    };
  ...
  return { keyboardHeight, isKeyboardOpen, browserBottomBar };
};
...
export const useMobileScroll = (elementRef: React.RefObject<HTMLElement>) => {
  useEffect(() => {
    ...
    return () => {
      ...
      (element.style as any).overflowScrolling = 'auto';
    };
  }, [elementRef]);
};
