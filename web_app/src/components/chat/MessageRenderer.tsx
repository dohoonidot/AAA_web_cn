 ...
 */
function parseMessage(message: string, archiveType: string): MessageParts {
  // ai, code, sap 타입은 CoT 표시 안함 (withModel API 사용)
  ...
  return (
    <Box
      sx={{
        ...
        boxShadow: isDark ? 'inset 0 1px 0 rgba(255, 255, 255, 0.05)' : 1,
      }}
    >
      {/* 헤더 */}
      <Box
      ...
      >
        ...
        <Typography
          variant="caption"
          sx={{ fontWeight: 'bold', fontSize: '0.875rem', color: colorScheme.markdownTextColor }}
        >
          {headerText}
        </Typography>
      </Box>
      ...
      <Box
        sx={{
          ...
          '&::-webkit-scrollbar-thumb': {
            bgcolor: isDark ? '#475569' : 'grey.400',
            borderRadius: '3px',
          },
        }}
      >
...
function ResponseDivider() {
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', my: 1 }}>
      <Box sx={{ flex: 1, height: '1px', bgcolor: colorScheme.textFieldBorderColor }} />
      <Typography
        variant="caption"
        sx={{
          px: 1.5,
 ...
 */
export default React.memo(function MessageRenderer({
  message,
  isStreaming = false,
  archiveType = 'company',
}: MessageRendererProps) {
  const { thoughtPart, responsePart, hasThoughtCompleted } = useMemo(
    () => parseMessage(message, archiveType),
    [message, archiveType]
  );
