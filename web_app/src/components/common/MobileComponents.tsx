 ...
 * 모바일 최적화된 카드 컴포넌트 (v2)
 */
export const MobileCardV2: React.FC<{
  children: React.ReactNode;
  onClick?: () => void;
  elevation?: number;
  sx?: any;
    ...
    >
      <Box
        ...
        sx={{
          ...
          maxHeight: shouldBeFullScreen ? '100%' : `calc(var(--app-height) - ${keyboardHeight + 64}px)`,
...
export const SwipeableListItem: React.FC<{
  ...
  // 스와이프 제스처 훅 사용
  const { onTouchStart, onTouchEnd } = useSwipeGesture(onSwipeLeft, onSwipeRight);
    ...
    >
      <Box
        sx={{
          display: 'flex',
          height: 64,
          // iOS 홈 인디케이터 / Android 제스처바 위에 겹치지 않도록
